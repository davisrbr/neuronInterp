...
class BasicBlockFixedBN(nn.Module):
    ...
    def __init__(
        ...
        self.bn1_0 = norm_layer(planes)
        self.bn1_1 = norm_layer(planes, affine=False)

        self.bn2_0 = norm_layer(planes)
        self.bn2_1 = norm_layer(planes, affine=False)
    ...
    def forward(self, xs):
        ...
        # out = self.conv1(x)
        # out = self.bn1(out)
        out, x_init = self.conv1_two_stream(x, self.bn1_0, self.bn1_1, x_init)
        ...
        out, x_init = self.conv2_two_stream(
            out, self.bn2_0, self.bn2_1, identity, downsample=self.downsample, x_init=x_init
        )
...
class ResNetFixedBN(nn.Module):
    def __init__(
        ...
        self.conv1_two_stream = ConvTwoStreamNormFixBN(
            3, self.inplanes, kernel_size=7, stride=2, padding=1, bias=False, mode=mode
        )
        self.bn1_0 = norm_layer(self.inplanes)
        self.bn1_1 = norm_layer(self.inplanes, affine=False)
    ...
    def _forward_impl(self, x):
        ...
        x, x_init = self.conv1_two_stream(x, self.bn1_0, self.bn1_1)
        # x = self.maxpool(x)
...
def resnet18_fixed(mode=0, **kwargs):
    ...
    """
    return _resnet_fixed("resnet18", BasicBlockFixedBN, [2, 2, 2, 2], mode, **kwargs)
...
def resnet32_fixed(mode=0, **kwargs):
    ...
    Args:
        mode (int): experiment mode
    """
    return _resnet_fixed("resnet32", BasicBlockFixedBN, [3, 4, 6, 3], mode, **kwargs)


def resnet18_sigmoid(mode=0, **kwargs):
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    return _resnet_sigmoid(
        "resnet18", BasicBlockSigmoidGating, [2, 2, 2, 2], mode, **kwargs
    )
...
def resnet32_sigmoid(mode=0, **kwargs):
    ...
    """
    return _resnet_sigmoid(
        "resnet32", BasicBlockSigmoidGating, [3, 4, 6, 3], mode, **kwargs
    )
...
if __name__ == "__main__":
    ...
    model = resnet18_fixed().to("cuda").eval()
    model(test_tens)
    # model = resnet18_sigmoid().to("cuda").eval()
    # model(test_tens)
    model = resnet32_fixed().to("cuda").eval()
    model(test_tens)
    # model = resnet32_sigmoid().to("cuda").eval()
    # model(test_tens)

    model = resnet18_fixed(mode=1).to("cuda").eval()
    model(test_tens)
    model = resnet32_fixed(mode=1).to("cuda").eval()
    model(test_tens)

    model = resnet18_fixed(mode=2).to("cuda").eval()
    model(test_tens)
    # model = resnet18_sigmoid(mode=2).to("cuda").eval()
    # model(test_tens)
    model = resnet32_fixed(mode=2).to("cuda").eval()
    model(test_tens)
    # model = resnet32_sigmoid(mode=2).to("cuda").eval()
    # model(test_tens)

    model = resnet18_fixed(mode=3).to("cuda").eval()
    model(test_tens)
    model = resnet32_fixed(mode=3).to("cuda").eval()
    model(test_tens)
