...
class ConvSigmoid(nn.Module):
    ...
    def __init__(
        self,
        in_channels, out_channels, kernel_size, bias=False, padding=1,
        mode=0
    ):
        super(ConvSigmoid, self).__init__()
        self.conv_weight = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias).cuda()


        self.conv_gating = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias).cuda()
...
class ConvTwoStream(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias).cuda()


        self.conv_init = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias).cuda()
...
class ConvTwoStreamNorm(nn.Module):
    ...
    def __init__(
        self,
        in_channels, out_channels, kernel_size, stride=1, bias=False, padding=1, mode=0
    ):
        super(ConvTwoStreamNorm, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias).cuda()


        self.conv_init = nn.Conv2d(in_channels, out_channels, stride=stride, kernel_size=kernel_size, padding=padding, bias=bias).cuda()
...
class ConvTwoStreamResidual(nn.Module):
    ...
    def __init__(
        self,
        in_channels, out_channels, kernel_size, stride=1, bias=False, padding=1,
        mode=0
    ):
        ...
        self.conv_init = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias).cuda()
...
class mCNN_fixed(nn.Module):
    ...
    def __init__(
        self,
        c=64, num_classes=10, mode=0
    ):
        super(mCNN_fixed, self).__init__()

        self.mode = mode

        self.conv1 = ConvTwoStream(3, c, kernel_size=3, padding=1, bias=True, mode = mode).cuda()

        self.conv2 = ConvTwoStream(c, c * 2, kernel_size=3, padding=1, bias=False, mode=mode).cuda()

        self.mp_2 = nn.MaxPool2d(2)
        self.mp_4 = nn.MaxPool2d(4)

        self.conv3 = ConvTwoStream(c * 2, c * 4, kernel_size=3, padding=1, bias=False, mode=mode).cuda()

        self.conv4 = ConvTwoStream(c * 4, c * 8, kernel_size=3, padding=1, bias=False, mode=mode).cuda()
